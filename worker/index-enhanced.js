/**
 * SANTRILOGY AI - SECURE BACKEND WORKER (LIVE v2.0)
    ...
    try {
      ...
      if (path === '/' || path === '/health') {
        return new Response(JSON.stringify({
          ...
          endpoints: ['/api/chat', '/api/history', '/api/session', '/api/auth']
        }), { 
      ...
      if (path === '/api/auth' && request.method === 'POST') {
        const result = await handleAuth(request, env, corsHeaders);
        return result;
      }
...
async function handleAuth(req, env, headers) {
  try {
    ...
    console.log(`Calling Firebase endpoint: ${endpoint}`);
    ...
    if (data.error) {
      console.error(`Auth error: ${JSON.stringify(data.error)}`);
    ...
    return errorResp(error.message, 500, headers);
  }
}
...
async function handleGoogleCallback(req, env) {
  ...
  try {
    ...
    if (firebaseData.error) throw new Error('Firebase login failed: ' + firebaseData.error.message);
    ...
    const finalUrl = `${bloggerUrl}/#token=${firebaseData.idToken}&user=${userSafe}`;
...
async function handleChat(req, env, headers) {
  try {
    ...
    // Kita gunakan ctx.waitUntil agar Worker tidak mati sebelum save selesai
    const savePromise = saveToFirestore(env, userId, sessionId, message, aiResponseText);
    ctx.waitUntil(savePromise); // Ini penting agar promise tak menghentikan response
  ...
  try {
    ...
    const history = (data || []).map(item => {
      ...
      return {
        ...
        title: fields.title?.stringValue || (fields.userMessage?.stringValue || "").substring(0, 50) + "..."
      };
    }).filter(Boolean);
...
async function handleSession(req, env, headers) {
  try {
    ...
    if (action === 'save') {
      ...
      return new Response(JSON.stringify({ success: true, message: 'Session saved successfully' }), { 
        headers: { ...headers, 'Content-Type': 'application/json' } 
      });
    } 
    else if (action === 'get') {
      const sessionData = await getSessionFromFirestore(env, userId, sessionId);
      return new Response(JSON.stringify({ success: true, data: sessionData }), { 
        headers: { ...headers, 'Content-Type': 'application/json' } 
      });
    } 
    else if (action === 'delete') {
      await deleteSessionFromFirestore(env, userId, sessionId);
      return new Response(JSON.stringify({ success: true, message: 'Session deleted successfully' }), { 
        headers: { ...headers, 'Content-Type': 'application/json' } 
      });
    } 
    else {
  ...
  try {
    ...
    const docData = {
      fields: {
        ...
        title: { stringValue: sessionData.title || (sessionData.messages?.[0]?.content || 'New Session').substring(0, 50) },
...
async function saveToFirestore(env, userId, sessionId, userMsg, aiMsg) {
  try {
    ...
    if (!res.ok) {
      const errorText = await res.text();
      console.error("Firestore Chat Save Error:", errorText);
    } else {
      console.log("Chat saved successfully to Firestore");
    }
  } catch (error) {
